...
- 코드 리더
- 주기적인 코드리뷰와 컨벤션, 컴포넌트, 폴더트리 등 가이드라인 제공
- 효율적인 역할분배 및 일정 조율
- 로고 디자인과 프로토타입 제공, 스플래시 화면 애니메이션 등 디자인 전반 담당

### **🎨 UI**

- 홈 피드 페이지, 프로필 페이지, 포스트 컴포넌트 UI 구현
- 로고, 페이지 내 기본 이미지, 스플래시 애니메이션 제작

### **🔧 기능 구현**

- 회원가입 페이지
  - 회원가입 데이터 전송
  - 계정/이메일 중복 및 입력 정보 유효성 검증
  - 프로필사진 첨부, 미리보기 및 버튼 활성화
- 홈 피드 페이지
  - 포스트 목록 데이터 요청
  - 로그인 여부에 따른 조건부 렌더링
  - 포스트 무한 스크롤(infinite scroll) 기능으로 사용성 개선
- 프로필 페이지
  - 유저 정보 및 포스트 목록 데이터 요청
  - 포스트 레이아웃 리스트뷰/앨범뷰
  - url 유저 검색 기능
- 팔로우/팔로잉 페이지
  - 팔로우/팔로잉 목록 데이터 요청
  - 팔로우/언팔로우 기능
  - 실시간 팔로우 count 업데이트 기능
- 포스트 상세페이지
  - 댓글 목록 데이터 요청
  - 댓글 작성 및 실시간 업데이트 기능
  - 댓글 무한 스크롤(infinite scroll) 기능으로 사용성 개선

### 🧩 기타

- token 검증
- useContext 이용하여 유저 정보 데이터 중복 요청 방지
- 모든 페이지 url로 접근 기능하도록 서비스 전체 라우팅 설계 및 구축 및 예외처리
- Pending 상태의 view 기능 추가하여 사용성 개선, 요청 실패시 notFound
- git flow, branch 사용 가이드 라인 제공
- 그 외 예외처리

  - 에러 이미지 기본 이미지 대체 기능
  - 포스트 상세페이지 내 Link 이동 기능 차단 (ex. 댓글 버튼 클릭시 포스트 상세페이지, 유저 프로필 클릭시 프로필 페이지)
