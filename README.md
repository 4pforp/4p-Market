...
- 팀원들 간의 원활한 소통 환경 제공 
  - 게더타운 활용하여 소통
  - 디스코드 -> 오늘 할 일 게시판을 통해 서로의 진행 상황 매일 파악, 응원 

### **🎨 UI**

- 모달창, 팝업창, 팔로잉/팔로워 목록, 채팅 목록, 채팅룸, 에러페이지 UI 구현
- 변경된 로고 이미지와 컬러로 피그마 제작
...
- 로그인 페이지
  ...
  - 페이지 진입 시 인풋창 포커스 기능 추가 구현
     - 클릭하지 않아도 input 창이 활성화되게끔 사용자 UX 측면 고려하여 구현
    
- 모달. 알림창
  - React portal 로 모달창 구현
  - 모달창, 팝업창 내 모든 기능 구현
  - Account 에 따라 페이지 별로 다른 모달창 화면 구현
 - 로그아웃 기능 구현 (API 명세에 따름)
   - localStorage 내 저장된 유저 정보 clear 
 - 삭제 기능 구현 (API 명세에 따름)
