...
- IR 기법을 포함한 전역에 사용되는 스타일 변수와 reset 스타일을 위한 Sass(SCSS)작업
- 이메일 회원가입 및 프로필 설정
- 이니셜 피드와 유저 검색창 이동 버튼
- 페이지 헤더 컴포넌트
- 검색창, 포스트 댓글 페이지
- 각 포스트 및 유저 피드에 나오는 유저 프로필,닉네임,계정이름을 하나로 컴포넌트화
- 뒤로가기/검색/좋아요/저장/더 보기/로그인 버튼 컴포넌트

### **🔧 기능 구현**

- 담당 기능 : 전체 페이지 헤더 컴포넌트 구현, 유저 검색 기능, 게시물 포스트 기능, 사용자 프로필 페이지 판매중인 상품 구현 기능, 좋아요 기능, 채팅 기능(예정)
- 프로젝트 작업 매니징을 위한 공유 노션 페이지 작성, 프로젝트 초기 작업 파일 세팅
...
- 코드 리더로 주기적인 코드리뷰와 컨벤션, 컴포넌트, 폴더트리 등등 가이드라인 제공 ,역할분배, 일정 조율
...
- 홈 피드 페이지
  - 포스트 목록
  - 로그인 여부에 따른 조건부 렌더링
  - 포스트 리스트 무한 스크롤(infinite scroll)
- 프로필 페이지
  - 유저 정보
  - 포스트 목록
  - 포스트 레이아웃 리스트뷰/앨범뷰
- 팔로우/팔로잉 페이지
  - 팔로우/팔로잉 목록
  - 팔로우/언팔로우 및 실시간 count 업데이트 기능
- 포스트 상세페이지
  - 댓글 목록
  - 댓글 작성 및 실시간 업데이트 기능
  - 댓글 리스트 무한 스크롤(infinite scroll)
